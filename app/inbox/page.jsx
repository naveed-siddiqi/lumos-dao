...
const Inbox = () => {
   ...
   const selectChat = async (chatParams) => {
       ...
       if(users[chatParams.user]) {
           ...
           socket.emit('isonline', {api_type:"main",user:chatParams.user}, (status) => {
       ...
       if(messages[chatParams.user]) {
           ...
           if(unread || bunread) {  
               socket.emit('read', {
                   api_type:"main", sender:currentUser, msgId:msgId, signal:unread
               }, (status) => { 
       ...
       if(currentUser != "") {
           ...
           if(msg != "") {
              ...
              if(currentUser.indexOf('Broadcast') == -1) {
                   ...
                   socket.emit('msg', {api_type:"main", msg:msg, receiver:currentUser}, (status) => {
                       //console.log(status)
                       if(status.status === true) {
                           //has sent
                  ...
                  p.receiver = 'all'
                   socket.emit('broadcast', {api_type:"main", msg:msg}, (status) => {
                       //console.log(status) 
                       if(status.status === true) { 
                           //has sent
                           p.id = status.id;p.status = 'sent'
   ...
   const doIsTyping = () => {
       ...
       if(currentUser.indexOf('Broadcast') == -1) {
           socket.emit('typing', {api_type:"main", receiver:currentUser}) //callback not needed
           ...
           typingTmr = setTimeout(() => {
               socket.emit('nottyping', {api_type:"main",receiver:currentUser})
           }, 8000)
   ...
   const addUser = async (res, after = true) => {
       ...
       if(!users_conversation.includes(u)) {
           ...
           if(!users[u]) {
               ...
               socket.emit('isonline', {api_type:"main", user:u}, (status) => {
