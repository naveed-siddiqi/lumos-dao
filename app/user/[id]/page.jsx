...
import {isChain, fAddr, getAllProposal, getAllUsersTx, getCommentNum, getDao, getDaoDelegatee, getDaoInfo, getDaoJoinedDate, getDaoJoinedWithInfo, getDaoMetadata, getProposal, getProposalVoterInfo, getUserComment, getUserInfo, getUserMeta } from '@/app/core/getter';
import DaoCard from '@/app/components/dao-card/DaoCard';
import { stopTalking, talk } from '@/app/components/alert/swal'; 
import DaoCardLoader from '@/app/components/dao-card-loader/DaoCardLoader';
import { PiWifiXLight } from 'react-icons/pi';
import AlertLoader from '@/app/components/alert-loader/AlertLoader';
import UserProfileLoader from '@/app/components/user-profile-loader/UserProfileLoader';
import CommentLoader from '@/app/components/comments-loader/CommentLoader';
import DelegateLoader from '@/app/components/delegate-loader/DelegateLoader';
...
const UserProfile = () => {
    ...
    const setUp = async () => { 
        ...
        if(userInfo.status) {
            ...
            if (userInfo?.cover) {
                setBackgroundImage(`url(${userInfo.cover})`);
              }
    ...
    const loadComments = async (comt) => {
        if (comt.length > 0) {
            ...
            E('wallet_my_comment').innerHTML = `
                                    ...
                                    <div class='my-[10px] mx-auto w-full'>
                                        <center class='flex flex-col items-center justify-center'>
                                            <img src='/images/no-comment.png' class='w-[70px] mb-2'/>
                                            <p class="text-[14px]">You have not made any comments</p>
                                        </center>
                                    </div>
                                `
        }
    ...
    const drawExp = (txInfo) => {
        ...
        const elem = `<div class='text-[12px] flex items-start justify-between border-[#EFEFEF] border-b p-3 my-3 flex-col md:items-center md:flex-row gap-3 overflow-x-scroll'>
  ...
  return (
    <div className='mb-[80px] helvetica-font'>
        ...
        <div className="bg-cover h-[370px] rounded-none relative" id='user_bg' style={{backgroundImage: backgroundImage, transition:'backgroundImage 200ms'}}>
            <div className='text-white text-[12px] md:mx-[6.5rem] mx-[1rem] px-[1.5rem] mt-[2.3rem] mb-5 hidden lg:inline-flex py-2 items-center gap-1 bg-[#84848459] relative z-10 rounded-full'>
                <a href="/dao">Home </a>
                <BsChevronRight />
                <p id='wallet_name'></p>
            </div>
        ...
        <div>
            <div className="flex items-start justify-between gap-3 md:ml-[6.5rem] mx-[1rem]">
                <div className='p-[4px] bg-[white] w-[85px] h-[85px] mt-[-40px] rounded-full relative z-[100]'>
                    <img className="w-full h-full rounded-full object-cover" id='user_img' alt="User profile Image" />
                </div>
                {
                    mobileNav ? 
                    <div onClick={() => setMobileNav(false)} className='cursor-pointer block lg:hidden px-2 py-[10px] bg-[#F6F6F8] rounded-[6px] mt-[-15px] relative z-[100]'>
                        <img src='/images/close.svg' /> 
                    </div>
                    :
                    <div onClick={() => setMobileNav(true)} className='cursor-pointer block lg:hidden px-2 py-[10px] bg-[#F6F6F8] rounded-[6px] mt-[-15px] relative z-[100]'>
        ...
        <div className='flex items-start lg:gap-[1rem] h-full sm:px-[2rem] px-[0.5rem] py-[1.8rem]'>
            ...
            <div className={mobileNav ? `fixed z-[100] left-0 top-0 flex justify-start py-8 flex-col border h-[100dvh] w-[300px] bg-white overflow-y-scroll` : `relative lg:flex justify-start py-8 flex-col h-full w-[30%] bg-white hidden`}>
            ...
            <div className='lg:w-[70%] w-full py-[3rem] px-[1.5rem] bg-[#FFFFFF] text-[12px]'>
                <div className='flex items-center text-center overflow-x-scroll'>
                ...
                <div id='wallet_my_dao' style={(selectedTab === 'joined')? {display:'grid'} : {display:'none'}}
                    ...
                    : (empty) ? 
                        <div style={{fontSize:'13px', margin:'10px 0px'}}>
