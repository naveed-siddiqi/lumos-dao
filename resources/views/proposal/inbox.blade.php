...
<main class="content proposal-inbox mt-5 border-0">
    ...
    <script>
        ...
        const searchUser = async (e) => {
            ...
            if(value == "") {
                E('searchList').style.display = 'none'
            }
            else {
                E('searchList').style.display = ''
                E('searchList').innerHTML = ""
                for(let i=0;i<users.length;i++) {
                    if(users[i].toLowerCase().indexOf(value.toLowerCase()) > -1) {
                        //present 
                        E('searchList').innerHTML += drawUser({
                            user:users[i]
                        })
                    }
                }
                if(E('searchList').innerHTML == "") {
                    E('searchList').innerHTML = `
        ...
        const drawUser = (params) => {  
            return `<a href="#" class="list-group-item list-group-item-action border-0 my-2">
                        <div class="d-flex align-items-start gap-3">
                            <div class="flex-grow-1 ml-3">
                            <img src="https://bootdey.com/img/Content/avatar/avatar3.png"
                                    class="rounded-circle mr-1" alt="" width="40" height="40">
                                ${params.user.substring(0,9) + "..." + params.user.substring(params.user.length-9)}
                                <div class="small" style='display:none'><span class="fas fa-circle chat-online"></span></div>
                            </div>
                        </div>
                    </a>`
        }
